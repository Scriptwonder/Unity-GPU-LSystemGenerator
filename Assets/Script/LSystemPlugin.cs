...
[Serializable]
public struct LSystemMap
{
    public string KeyString;
    public GameObject ValueGameObject;
    public float length;//the y length of that symbol
    public float scale;
}

[Serializable]
public class LSystemPlugin : MonoBehaviour
{
    ...
    public List<GPURule> rules = new List<GPURule>();
    public List<LSystemMap> LSystemDefiner = new List<LSystemMap>();
    ...
    private Dictionary<String, MeshDataStruct> meshPosData = new Dictionary<string, MeshDataStruct>();
    ...
    private float[] lengthData = new float[100 * 4];
    ...
    void prepareMapData(List<LSystemMap> map)
    {
        foreach (LSystemMap mapContent in map)
        {
            MeshDataStruct newData = new MeshDataStruct();
            Mesh mesh1 = mapContent.ValueGameObject.GetComponent<MeshFilter>().sharedMesh;
            newData.mesh = mesh1;
            newData.id = mapContent.ValueGameObject.name;
            newData.referenceToObj = mapContent.ValueGameObject;
            newData.mat = mapContent.ValueGameObject.GetComponent<MeshRenderer>().sharedMaterial;
            newData.scale = mapContent.scale == 0f ? 1f : mapContent.scale;
            if (!meshPosData.ContainsKey(newData.id))
            {
                meshPosData.Add(mapContent.KeyString, newData);
                int asciiCode = charToAscii(newData.id[0]);
                lengthData[asciiCode * 4] = mapContent.length;
            } else
            {
                Debug.LogError("A key with same id is already added");
            }

        }
    }//prepare the data in LSystemDefiner to be used in compute shader and in interpretation

    void drawInstantiate()//used currently
    {
        for (int i = 0; i < meshNameData.Count; i++) // for each characer in derived string
        {
            string curChar = meshNameData[i];
            MeshDataStruct meshD = meshPosData[meshNameData[i]];
            ...
            newObj.transform.localScale = newObj.transform.localScale * meshD.scale;
        }
    ...
    void Awake()
    {
        ...
        prepareMapData(LSystemDefiner);
        int lengthId = Shader.PropertyToID("_lengthBuffer");
        computeShader.SetFloats(lengthId, lengthData);
